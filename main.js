...
const baniosQuimicos = [
  ...
  function buscarPrecioPorCantidad(cantidad) {
    return baniosQuimicos.find(banio => banio.cantidad === cantidad)?.precio;
  }
  ...
  function calcularTotal(cantidadBanios, dias, serviciosMantenimiento, pagos, tarjetaCuotas) {
    const PRECIOXDIA = 5000;
    let total = 0;
  
    if (validarCantidadDias(dias)) {
      let precioBanio = buscarPrecioPorCantidad(cantidadBanios);
  ...
  function validarFormulario() {
    const cantidadBanios = obtenerInputNumero("cantidadBaniosInput");
    ...
    if (cantidadBanios >= 2 && !isNaN(cantidadBanios) && validarCantidadDias(dias) && validarRespuestaSiNo(serviciosMantenimiento) && validarRespuestaSiNo(pagos)) {
      const total = calcularTotal(cantidadBanios, dias, serviciosMantenimiento, pagos, tarjetaCuotas);
